...
async function sendBookingEmail(bookingDetails, paymentDetails) {
  const adminEmail = "indrakantivenkatagopalakrishna@gmail.com";
  
  const mailSubject = `New Booking Confirmed: ${bookingDetails.pooja_name}`;
  const mailBody = `
Hello,

A new pooja booking has been confirmed!

--- BOOKING DETAILS ---
Booking ID: ${bookingDetails.id}
Pooja Name: ${bookingDetails.pooja_name}
Customer Name: ${bookingDetails.customer_name}
Customer Email: ${bookingDetails.customer_email}
Customer Phone: ${bookingDetails.customer_phone}
Gotra: ${bookingDetails.gotra || 'N/A'}
Rashi: ${bookingDetails.rashi || 'N/A'}
Booking Date: ${bookingDetails.booking_date}

--- PAYMENT DETAILS ---
Payment ID: ${bookingDetails.payment_id || paymentDetails.razorpay_payment_id || 'N/A'}
Order ID: ${paymentDetails.razorpay_order_id || 'N/A'}
Amount Paid: ₹${paymentDetails.amount || '2000.00'}
Status: Confirmed / Captured

Regards,
Temple Booking Platform
`;
  ...
  try {
    ...
    console.log("Booking confirmation email sent successfully. Message ID:", result.MessageId);
  } catch (err) {
...
app.post('/payment/create-order', async (req, res) => {
  const { booking_id, customer_name, customer_email, customer_phone } = req.body;
  if (!booking_id) {
    return res.status(400).json({ error: 'booking_id is required' });
  }

  try {
    let customerName = customer_name || 'Guest User';
    let customerEmail = customer_email || 'guest@example.com';
    let customerPhone = customer_phone || '9999999999';

    // 1. Fetch booking details (wrapped in independent try-catch)
    try {
      ...
      console.warn('Database booking fetch failed, using fallback customer info:', dbErr.message);
    }
    ...
    // 3. Save order to payments table (wrapped in independent try-catch)
    try {
      await db.query(
        ...
        [booking_id, orderId, 2000.00]
      );
    } catch (dbErr) {
      console.warn('Database payment creation recording failed, continuing:', dbErr.message);
    }
    ...
    console.warn('Razorpay order creation call failed. Returning mock payment order.');
...
app.post('/payment/verify', async (req, res) => {
  ...
  try {
    ...
    sendBookingEmail(booking, { razorpay_payment_id, razorpay_order_id, amount: 2000.00 }).catch(err => {
    ...
    if (event === 'payment.captured' || event === 'order.paid') {
      ...
      if (paymentRes.rows.length > 0) {
        ...
        if (bookingRes.rows.length > 0 && bookingRes.rows[0].status === 'pending') {
          ...
          // Send email notification to user
          sendBookingEmail(bookingRes.rows[0], { razorpay_payment_id: paymentId, razorpay_order_id: orderId, amount: 2000.00 }).catch(err => {
            console.error('Failed to send email notification:', err);
          });
        }
      }
